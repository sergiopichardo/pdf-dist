...
def random_component_by_score(component_type, component_map):
    ...
    avg_scores = {}
    for name in names: 
        score = int(values.get(name, 1)) # `dictionary.get(key, default_value)`
...
def score_conversation(
    ...
    client.hincrby("llm_score_values", llm, score)  # hash increment by - hincrby
    client.hincrby("llm_score_counts", llm, 1) 

    client.hincrby("retriever_score_values", retriever, score)  # hash increment by - hincrby
    client.hincrby("retriever_score_counts", retriever, 1) 

    client.hincrby("memory_score_values", memory, score)  # hash increment by - hincrby
